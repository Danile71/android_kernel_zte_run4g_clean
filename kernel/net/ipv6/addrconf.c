...
static void addrconf_dad_stop(struct inet6_ifaddr *ifp, int dad_failed)
{
	if (ifp->flags&IFA_F_PERMANENT) {
		...
		if(dad_failed)
			...
			pr_warning("[mtk_net]skip ipv6_del_addr(temp addr) if dad_failed==0 \n");
#endif
	} else {
		if(dad_failed)
			ipv6_del_addr(ifp);
		else
			pr_warning("[mtk_net]skip ipv6_del_addr if dad_failed==0 \n");
	}
}
...
void addrconf_prefix_rcv(struct net_device *dev, u8 *opt, int len, bool sllao)
{
	...
	if (!(in6_dev->if_flags & IF_READY)) {
		pr_warning("[mtk_net] get prefix but device %s is not ready\n", dev->name);
	}
